...
class DensityOfStates(Dispersion, DataContainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "pdos" in data and len(set(proj[2] for proj in data["pdos"]["projectors"])) == 2:
            ...
            if np.max(data["pdos"]["energies"]) < 0 or np.min(data["pdos"]["energies"]) > 0:
...
"""
        ...
        if 'dos' not in data and 'pdos' in data:
            data["dos"] = np.sum(data["pdos"][proj] for proj in data["pdos"])
        ...
        if 'spin_dos' not in data and 'pdos' in data:
            spin_channels = set(proj[2] for proj in data["pdos"])
            if len(spin_channels) == 2:
                data["spin_dos"] = {}
                for channel in spin_channels:
                    data["spin_dos"][channel] = np.sum(data["pdos"][proj] for proj in data["pdos"] if proj[2] == channel)
